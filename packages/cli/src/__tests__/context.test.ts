...
describe("resolveConfigDir", () => {
  it("respects ALTOCUMULUS_CONFIG_DIR override", () => {
    const prev = process.env["ALTOCUMULUS_CONFIG_DIR"];
    process.env["ALTOCUMULUS_CONFIG_DIR"] = "/tmp/cli-override";
    try {
      expect(resolveConfigDir()).toBe("/tmp/cli-override");
    } finally {
      if (prev === undefined) delete process.env["ALTOCUMULUS_CONFIG_DIR"];
      else process.env["ALTOCUMULUS_CONFIG_DIR"] = prev;
    }
  });
});
