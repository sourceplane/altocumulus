// CLI error helpers. Translate `AltocumulusError` subclasses (from
...
export class MissingOrgContextError extends Error {
  constructor() {
    super("no active organization (run `altocumulus org use <id>`)");
...
export function formatCliError({ err, mode }: FormatErrorInput): FormattedError {
  ...
  if (err instanceof UnauthenticatedError) {
    return formatSdk(4, err, "token rejected — run `altocumulus login` to refresh", mode);
  }
  if (err instanceof AltocumulusError) {
