...
`altocumulus` — first-class TypeScript CLI for the Altocumulus control
...
This package is internal to the `altocumulus` monorepo and is not
...
```
altocumulus login    [--api-url=URL] [--token=BEARER]
altocumulus logout
altocumulus whoami
```
...
```
altocumulus org list
altocumulus org use <org-id>
altocumulus org members
altocumulus project list
```

Writes (Task 0101):

```
altocumulus org invite <email> [--role=ROLE] [--idempotency-key=KEY] [--org=ORG_ID]
altocumulus project create <name> [--idempotency-key=KEY]
altocumulus env create <project-id> <name> [--idempotency-key=KEY]
altocumulus api-key create <name> [--scope=SCOPE] [--idempotency-key=KEY]
altocumulus webhook create <url> [--event=EVENT[,EVENT2,...]] [--idempotency-key=KEY]
```

Cross-resource reads (Task 0101):

```
altocumulus usage summary    [--metric=METRIC] [--from=ISO] [--to=ISO]
altocumulus billing summary
altocumulus audit list       [--limit=N] [--cursor=CURSOR] [--category=CAT] [--all]
```
...
context (`altocumulus org use <org-id>`). Only `org invite` accepts an
...
The shipped V1 is **token-paste**: `altocumulus login` prompts for a
...
- `FileTokenStore` fallback: `~/.config/altocumulus/credentials.json`,
  mode **0600**, parent directory mode **0700**.

Active organization context lives at
`~/.config/altocumulus/config.json` (mode 0644, not a secret). Override
both via `ALTOCUMULUS_CONFIG_DIR` (used by tests).
