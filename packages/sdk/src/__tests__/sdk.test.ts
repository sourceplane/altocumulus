...
import { Altocumulus } from "../index.js";
import {
  BadRequestError,
  ConflictError,
  ForbiddenError,
...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new Altocumulus({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => AltocumulusError]> = [
  ...
  it("forward-compat: unknown error code → base AltocumulusError", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(AltocumulusError);
      expect(err).not.toBeInstanceOf(BadRequestError);
      expect((err as AltocumulusError).code).toBe("future_quota_exceeded");
    }
  });
