// Base HTTP transport for the Altocumulus SDK.
//
// Goals:
...
// - Typed errors: every non-2xx response is decoded into a `AltocumulusError`
...
export interface ClientOptions {
  /**
   * Base URL of the Altocumulus api-edge worker, e.g.
   * `https://api.altocumulus.dev`. Trailing slash is stripped.
   */
  baseUrl: string;
 ...
 * `Altocumulus` (the client class wired up in `index.ts`).
 */
export class Transport {
  ...
  constructor(options: ClientOptions) {
    ...
    if (typeof fetchImpl !== "function") {
      throw new TypeError(
        "Altocumulus SDK requires a fetch implementation. Pass `fetch` in options on platforms without a global.",
      );
    }
...
export function generateRequestId(): string {
  ...
  throw new Error(
    "Altocumulus SDK requires the Web Crypto API (globalThis.crypto). No source of randomness was found.",
  );
}
