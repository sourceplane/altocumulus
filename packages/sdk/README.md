...
Altocumulus TypeScript SDK — typed, dependency-free, runtime-agnostic client
for the Altocumulus control plane API.

## Install (workspace)

This package is internal to the `altocumulus` monorepo. Add it as a
...
```ts
import { Altocumulus } from "@saas/sdk";

const client = new Altocumulus({
  baseUrl: "https://api.altocumulus.dev",
  auth: { kind: "bearer", token: process.env["ALTOCUMULUS_TOKEN"]! },
});
...
All non-2xx responses throw a typed subclass of `AltocumulusError`. Branch on
...
| `AltocumulusError` (base)   | _any_                 | _any_          |

Unknown error codes (forward-compatible — e.g. a future `quota_exceeded`)
decode to the base `AltocumulusError` carrying the raw envelope. Non-JSON 5xx
...
A custom `fetch` implementation can be injected via `new Altocumulus({ fetch })`.
